...
Revision 5 (NIST 800-53r5) custom ETL
"""

import re
from sckg.etl.generic import Generic

class NIST80053r5(Generic):
  """NIST 800-53r5 custom ETL"""

  def __init__(self, config):
    super().__init__(config)

  def extract(self, regime, parsable_document):
    """NIST 800-53r5 extract method
    ...
    for control_dict in baseline_list:
      ...
      name = control_dict['control_identifier']
      name_fields = name.replace(')', '').split('(')
      if len(name_fields) > 1:
        name = '{} ({})'.format(name_fields[0], name_fields[1])
  ...
  def transform(self, regime, regime_list):
    """NIST 800-53r5 custom transform method
    ...
    # 800-53r5 includes per-control metadata on so-called 'related controls'
    ...
    # currently 800-53r5 is the only regime that has this config attribute due
  ...
  def load(self, regime, neo4j, stmts):
    """NIST 800-53r5 custom load method
